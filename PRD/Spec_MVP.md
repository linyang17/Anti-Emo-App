...
## 🧱 2. 数据模型层（Models）
  ...
  **Item**           `Core/Models/Item.swift`           `sku, type, name, costEnergy, moodBoost, hungerBoost`。                   可用于购买/喂食逻辑。

  **UserStats**      `Core/Models/UserStats.swift`      `totalEnergy, coins, streakDays, lastActiveDate, completedTasksCount`。   能记录连续天数、能量值、金币变化。
  --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
## ⚙️ 3. 服务层（Services）
  ...
  **TaskGeneratorService**   `Core/Services/TaskGeneratorService.swift`   根据天气和历史任务生成今日任务；避免重复；3--6   每天仅生成一批新任务；不同天气模板正确切换。
                                                                          项任务。                                         
  ...
  **NotificationService**    `Core/Services/NotificationService.swift`    请求通知权限；安排每日 8:00 / 20:30 提醒。       本地通知能按时触发；未授权时有 fallback 提示。
...
## 🧩 4. 各功能模块（Features）
  ...
  **Pet**            `/Features/Pet/`          展示宠物形象、心情、饱食度；交互（摸头/喂食）。   点击交互宠物表情或数据实时变化。
