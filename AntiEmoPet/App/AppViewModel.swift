...
// MARK: Pet actions
	...
	private func purgePettingCounts(_ dict: [String: Int], keeping day: String) -> [String: Int] {
		...
		private func historyRange(forDays days: Int) -> ClosedRange<Date> {
			...
			guard stats.shareLocationAndWeather else {
				UserDefaults.standard.set(yesterdayKey, forKey: lastSuccessfulKey)
				logger.info("Skipped upload (sharing disabled). Marked lastSuccessfulDay=\(yesterdayKey, privacy: .public)")
				return
			}

			// Determine where to start backfilling.
			// If never uploaded before, start from yesterday only (avoid large historical backfill).
			let storedLast = UserDefaults.standard.string(forKey: lastSuccessfulKey)
			let startDate: Date
			if let storedLast, let parsed = isoDayFormatter.date(from: storedLast) {
				startDate = cal.date(byAdding: .day, value: 1, to: cal.startOfDay(for: parsed)) ?? yesterday
			} else {
				startDate = yesterday
			}

			// Nothing to do if start is after yesterday.
			guard startDate <= yesterday else { return }

			func uploadSummariesAsync(_ summaries: [UserTimeslotSummary]) async -> Bool {
				await withCheckedContinuation { continuation in
					uploadService.uploadTimeslotSummaries(summaries) { ok in
			...
			while cursor <= yesterday {
				...
				if tasks.isEmpty && moods.isEmpty {
					UserDefaults.standard.set(cursorKey, forKey: lastSuccessfulKey)
					logger.debug("No data for \(cursorKey, privacy: .public); marked processed.")
				...
				let ok = await uploadSummariesAsync(summaries)
				if ok {
					UserDefaults.standard.set(cursorKey, forKey: lastSuccessfulKey)
