...
struct GrowingTextView: UIViewRepresentable {
		...
		func updateUIView(_ uiView: UITextView, context: Context) {
				// Sync external text changes into the UITextView
				let textDidChange = (uiView.text != text)
				if textDidChange {
						uiView.text = text
				}

				// Manage focus / first responder state
				...
				// Only recalculate height when the text actually changed from SwiftUI side.
				if textDidChange {
